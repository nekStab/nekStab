...
!
! Dependencies:
!   nekstab_krylov_subspace, SIZE, TOTAL
!-----------------------------------------------------------------------
module nekstab_modal_dmd

   use nekstab_krylov_subspace, only: krylov_vector, k_zero, k_add2s2, k_norm, &
                                      NEKSTAB_PI
   use nekstab_krylov_inner_products, only: k_gram_matrix, k_project
   use nekstab_lapack, only: eig_symmetric, eig
   use nekstab_vectors, only: nopcopy
   use nekstab_nek_bridge, only: nid, vx, vy, vz, pr, t, nekStab_dp, &
                                 NEKSTAB_UNIT_DMD, modal_dt
...
!        Write SVD diagnostics (before sqrt overwrites S)
      if (nid == 0) then
         open (unit=NEKSTAB_UNIT_DMD, file='dmd_svd.dat', status='replace')
         write (NEKSTAB_UNIT_DMD, '(A)') '# DMD SVD Singular Values'
         write (NEKSTAB_UNIT_DMD, '(A,I6,A,F6.2,A)') &
            '# rank_used = ', r, '  (', &
            100.0d0*sum(S(1:r))/total_energy, '% energy)'
         write (NEKSTAB_UNIT_DMD, '(A,I6)') &
            '# total_snapshots = ', n
         write (NEKSTAB_UNIT_DMD, '(A)') &
            '# i       sigma_i         energy_%    cumulative_%'

         cumsum = 0.0d0
         do i = 1, n
            cumsum = cumsum + S(i)
            write (NEKSTAB_UNIT_DMD, '(I6, 3E16.8)') i, sqrt(S(i)), &
               100.0d0*S(i)/total_energy, &
               100.0d0*cumsum/total_energy
         end do

         close (NEKSTAB_UNIT_DMD)
...
!        Reconstruct modes and write spectrum
 ...
 &   dmd_norms, min(nsave, r))
   ...
   subroutine dmd_write_spectrum(evals, r, delta_t, norms, nnorms)
      ...
      open (unit=NEKSTAB_UNIT_DMD, file='dmd_spectrum.dat', status='replace')
      write (NEKSTAB_UNIT_DMD, '(A)') '# DMD Eigenvalue Spectrum'
      write (NEKSTAB_UNIT_DMD, '(A)') '# mode   |mu|      sigma       omega       ' &
      ...
      do m = 1, r
         ...
         if (m <= nnorms) then
            write (NEKSTAB_UNIT_DMD, '(I6, 7E14.6)') m, mu_mag, sigma, omega, &
 &              freq, real(evals(m)), aimag(evals(m)), norms(m)
         else
            write (NEKSTAB_UNIT_DMD, '(I6, 6E14.6)') m, mu_mag, sigma, omega, &
