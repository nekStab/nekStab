...
module nekstab_newton
   use nekstab_krylov_subspace, only: allocate_orbit, dnekclock, fc_nwt, ic_nwt, k_add2, &
   ...
   use nekstab_nek_bridge, only: bm1s, dt, ew_tol_cap, if3D, if3d, ifdyntol, iffindiff, &
                                 ...
                                 NEKSTAB_UNIT_ARNOLDI_LOG, NEKSTAB_UNIT_GMRES_LOG, &
                                 NEKSTAB_UNIT_NEWTON_LOG, nid, nof, nsteps, param, pbase, &
                                 pr, restol, t, tbase, time, ubase, uparam, vbase, vx, vy, &
                                 vz, wbase, wdsize
   use nekstab_vectors, only: nopcopy
   use nekstab_matvec, only: cache_newton_bvec, matvec, prepare_linearized_solver
   use nekstab_krylov_decomposition, only: arnoldi_factorization
   use nekstab_lapack, only: lstsq
   ...
   integer, parameter :: newton_log_unit = NEKSTAB_UNIT_NEWTON_LOG
   integer, parameter :: gmres_log_unit = NEKSTAB_UNIT_GMRES_LOG
   integer, parameter :: arnoldi_log_unit = NEKSTAB_UNIT_ARNOLDI_LOG
contains
...
! --> Relaxed exit condition if finite-difference approximation of the operator is considered.
            if ((iffindiff) .and. (beta2 < 1.0d-8)) then ! count of calls to linearized solver
               exit arnoldi
            end if
         end do arnoldi
         ...
         if ((iffindiff) .and. (beta2 < 1.0d-6)) then
            exit gmres
         end if
      end do gmres
