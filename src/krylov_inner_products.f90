...
c        Allocate velocity-sized workspace
         allocate(Aw(lv, m), B(lv, n), wk_gop(m*n))

c        ── vx (beta=0 initializes G) ──
         do i = 1, m
...
c        ── Temperature fields (lt-sized workspace) ──
...
c        ── Newton PO time component (scalar, no MPI) ──
         if (isNewtonPO) then
            do j = 1, n
               do i = 1, m
...
c        ── Single global reduction ──
      ...
      subroutine k_gram_complex(CSD, blk_re, blk_im, nblk, ldcsd)
         ...
         complex(kind=kind(0.0d0)), intent(out) :: CSD(ldcsd, nblk)
...
c        4 Gram matrices packed contiguously for single gop
...
c        Velocity-sized workspace (reused for each component)
         allocate(Aw(lv, nblk), B(lv, nblk))

c        ── Process each velocity field ──
c        vx
...
c        ── Pack into contiguous array for single gop ──
         off = 0
         do j = 1, nblk
            do i = 1, nblk
               off = off + 1
...
c        ── G_ri: <re, im> (regather Aw=re*bm1s, overwritten by G_ii) ──
         do i = 1, nblk
            if (field == 'x') then
               call copy(Aw(1,i), blk_re(i)%vx, nv)
               call col2(Aw(1,i), bm1s, nv)
...
c        B already has im from G_ii
         call dgemm('T','N', nblk, nblk, nv,
     $        1.0d0, Aw, lv, B, lv, dbeta, G_ri, nblk)

c        ── G_ir: <im, re> ──
c        Aw = im*bm1s (already gathered for G_ii)
         do i = 1, nblk
            if (field == 'x') then
               call copy(Aw(1,i), blk_im(i)%vx, nv)
               call col2(Aw(1,i), bm1s, nv)
