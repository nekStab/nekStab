...
!
! Dependencies:
!   nekstab_krylov_subspace, nekstab_fourier, nekstab_modal_pod, SIZE, TOTAL
!-----------------------------------------------------------------------
module nekstab_modal_spod

   use nekstab_krylov_subspace, only: krylov_vector, k_zero, k_dot, k_add2s2, k_cmult, &
                                      k_norm, nv, nt
   ...
   use nekstab_fourier, only: fft_frequencies, fft_r2c
   use nekstab_modal_pod, only: hamming_window
   use nekstab_nek_bridge, only: nid, nekStab_error, nekStab_dp, &
                                 NEKSTAB_UNIT_SPOD, if3d, ifto, vx, vy, vz, pr, t
...
contains
   ...
   subroutine spod_compute(snaps, nsnap, delta_t, nfft, noverlap, nsave)
      ...
      if (nblk < 2) then
         call nekStab_error('Not enough snapshots for SPOD (need >=2 blocks)')
      end if
      ...
      !        Open spectrum file
      if (nid == 0) then
         open (unit=NEKSTAB_UNIT_SPOD, file='spod_spectrum.dat', status='replace')
         write (NEKSTAB_UNIT_SPOD, '(A)') '# SPOD Spectrum: St, eigenvalues (nblk columns)'
      end if
...
!           Form Hermitian CSD matrix (batch)
         ...
         if (nid == 0) then
            write (NEKSTAB_UNIT_SPOD, '(E14.6)', advance='no') freq(ifreq)
            do i = 1, nblk
               write (NEKSTAB_UNIT_SPOD, '(E14.6)', advance='no') spod_evals(i)
            end do
   ...
   subroutine fft_block_at_freq(snaps, blk_start, nfft, &
      ...
      ! nv/nt from globals (nekstab_krylov_subspace)
