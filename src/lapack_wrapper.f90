...
module nekstab_lapack
   use nekstab_nek_bridge, only: nekStab_dp, nekStab_error
   implicit none
   private
...
!-----------------------------------------------------------------------
   subroutine eig_symmetric(A, eigvals, eigvecs, n)
      ...
      if (info /= 0) then
         call nekStab_error('DSYEV failed with info = '//trim(adjustl(transfer(info, '    '))))
      end if
...
!-----------------------------------------------------------------------
   subroutine eig_hermitian(A, eigvals, eigvecs, n)
      ...
      if (info /= 0) then
         call nekStab_error('ZHEEV failed with info')
      end if
