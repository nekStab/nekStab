...
!
! Dependencies:
!   nekstab_vectors, nekstab_nek_bridge, nekstab_io, nekstab_matvec,
!   nekstab_eigensolvers, krylov_subspace (subroutine-level)
!
! See also:
!   docs/pke-periodic-orbit.md — Floquet PKE budget derivation
!-----------------------------------------------------------------------

module nekstab_energy_budget
   use nekstab_vectors, only: nopcopy, nopcmult
   use nekstab_nek_bridge, only: nid, SESSION, vx, vy, vz, pr, t, &
                                 ubase, vbase, wbase, pbase, tbase, maxmodes, bm1, if3d, ifto, &
                                 NEKSTAB_UNIT_PKE, nelv, lelv, lelt, lx1, ly1, lz1, nsteps, dt, time, &
                                 istep, ifpert, ifadj, lsize, ifbase, param, vxp, vyp, vzp, tp
   use nekstab_io, only: read_eigenvalue
   use nekstab_matvec, only: prepare_linearized_solver
   use nekstab_eigensolvers, only: norm
   implicit none
   private
...
contains
   ...
   subroutine ensure_dissipation_workspace()
      ...
      !  WHY: allocate once (fixed lv) at first use; prevents per-call
      !  heap traffic inside the per-mode budget loops (historical pattern).
   ...
   subroutine stability_energy_budget
      ...
      do mode = 1, maxmodes
         ...
         if (nid == 0) then
            ...
            open (NEKSTAB_UNIT_PKE, file=filename, form='formatted')
            do i = 1, 10
               write (NEKSTAB_UNIT_PKE, '(1E15.7)') integrals(i)
               write (*, *) 'Integral ', i, ' = ', integrals(i)
            end do
            write (NEKSTAB_UNIT_PKE, '(1E15.7)') sum(integrals(1:9))
...
!     = -Dbar (it is negative), so the physical P-D is sum(1:9) + integrals(10).
            write (NEKSTAB_UNIT_PKE, '(1E15.7)') sum(integrals(1:9)) + integrals(10)
            close (NEKSTAB_UNIT_PKE)
         end if ! nid == 0
   ...
   !-----------------------------------------------------------------------
   ! stability_energy_budget_floquet — Orbit-averaged PKE budget for Floquet
   !
   ! Purpose:
   ...
   subroutine stability_energy_budget_floquet
      use nekstab_krylov_subspace, only: lv, lt, nv, nt, allocate_orbit, &
                                         orbit_store, orbit_restore, uor, vor, wor, tor
      ...
      !         krylov_subspace globals (set at init; see krylov_subspace.f90).
...
!     (negative), so the physical P-D is sum(1:9) + integrals(10).
            write (NEKSTAB_UNIT_PKE, '(1E15.7)') &
               sum(integrals(1:9)) + integrals(10)
            write (NEKSTAB_UNIT_PKE, '(1E15.7)') E_bar
            write (NEKSTAB_UNIT_PKE, '(1E15.7)') sigma_check
            close (NEKSTAB_UNIT_PKE)
         end if
   ...
   subroutine compute_dissipation(vx_dRe, vy_dRe, vz_dRe, vx_dIm, vy_dIm, vz_dIm, dissipation)
      ...
      dissipation = 0.5d0*dissipation*param(2)/param(1)
   ...
   subroutine compute_production(vx_dRe, vy_dRe, vz_dRe, vx_dIm, vy_dIm, vz_dIm, component, prod_x, prod_y, prod_z)
      ...
      if (component == 1) then
         ...
         prod_x = -0.5d0*(vx_dRe**2 + vx_dIm**2)*prod_dcdx_s
         prod_y = -0.5d0*(vx_dRe*vy_dRe + vy_dIm*vx_dIm)*prod_dcdy_s
         if (if3d) prod_z = -0.5d0*(vx_dRe*vz_dRe + vz_dIm*vx_dIm)*prod_dcdz_s

      else if (component == 2) then
         call gradm1(prod_dcdx_s, prod_dcdy_s, prod_dcdz_s, vbase, nelv)

         prod_x = -0.5d0*(vx_dRe*vy_dRe + vy_dIm*vx_dIm)*prod_dcdx_s
         prod_y = -0.5d0*(vy_dRe**2 + vy_dIm**2)*prod_dcdy_s
         if (if3d) prod_z = -0.5d0*(vy_dRe*vz_dRe + vz_dIm*vy_dIm)*prod_dcdz_s

      else if (component == 3) then
         if (if3d) then
            call gradm1(prod_dcdx_s, prod_dcdy_s, prod_dcdz_s, wbase, nelv)

            prod_x = -0.5d0*(vx_dRe*vz_dRe + vz_dIm*vx_dIm)*prod_dcdx_s
            prod_y = -0.5d0*(vy_dRe*vz_dRe + vz_dIm*vy_dIm)*prod_dcdy_s
            prod_z = -0.5d0*(vz_dRe**2 + vz_dIm**2)*prod_dcdz_s
         end if
      end if
