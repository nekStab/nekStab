...
!
! Dependencies:
!   krylov_subspace, nekstab_nek_bridge, nekstab_vectors, nekstab_io,
!   nekstab_eigensolvers, nekstab_matvec, nekstab_diagnostics,
!   nekstab_newton, nekstab_torque_mod, nekstab_energy_budget
!-----------------------------------------------------------------------
...
module nekstab_sensitivity
   ...
   use nekstab_eigensolvers, only: inner_product, norm
   use nekstab_matvec, only: prepare_linearized_solver
   use nekstab_diagnostics, only: smooth_field
   use nekstab_newton, only: ts_gmres
   use nekstab_torque_mod, only: nekStab_torque
   implicit none
   private
...
contains
   ...
   !     or any other scalar.
   !
   !  Note: historical path; the wavemaker outpost convention (wm_*) preserves
   !  v1.0 user expectation for post-processing scripts.
   !-----------------------------------------------------------------------
...
!     creating Fortran temporaries) hits the garbage tail.
   ...
   !
   !  Note: historical path; thin wrapper kept for call sites in user-facing
   !  uparam(01)=4.41/4.42 paths.
   !-----------------------------------------------------------------------
   ...
   subroutine biorthogonalize( &
      ...
      if (alpha + beta < 1.0d-60) then
         call nekStab_log('WARNING: zero-norm direct mode in biorthogonalize')
         return
      end if
      gamma = 1.0d0/sqrt(alpha + beta)
      ...
      if (gamma**2 + delta**2 < 1.0d-60) then
         call nekStab_log('WARNING: degenerate adjoint-direct pairing in biorthogonalize')
         return
      end if
...
! Loop over num_steps to create snapshots
   ...
   subroutine compute_omegaR(vx_in, vy_in, vz_in, omegaR)
      ...
      if (optimal_eps >= 1.0d0) then
         ...
         if (nid == 0) write (6, *) 'nekStab: Initial eps = ', eps
         ...
         do iter = 1, 20
            ...
            if (abs(omegaR_max - 1.0d0) < 1.0d-8) exit

            omegaR_min = glmin(omegaR, nv)
            call nekStab_log('Iteration omegaR min,max:')
            if (nid == 0) write (6, *) iter, ':', omegaR_min, omegaR_max

            if (omegaR_max > 1.0d0) then
               call nekStab_log('Increasing eps:')
               if (nid == 0) write (6, *) eps
               eps = eps*2.0d0
            else if (omegaR_max < 1.0d0) then
               call nekStab_log('Decreasing eps:')
               if (nid == 0) write (6, *) eps
               eps = eps*0.50d0
            end if

            optimal_eps = eps
            call nekStab_log('Final eps value:')
            if (nid == 0) write (6, *) optimal_eps
...
! compute for fintim and not period, as we might have n periods
         ...
         call nekStab_log('Maximum spatial restriction:')
         if (nid == 0) write (6, *) ctarg
         ...
         do istep = 1, nsteps
            ...
            if (ifoutfld) then
               ...
               call nekStab_log('Amplitude:'); if (nid == 0) write (6, *) A0, amplitude
