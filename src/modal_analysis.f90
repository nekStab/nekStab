...
!
! Dependencies:
!   nekstab_krylov_subspace, nekstab_vectors, nekstab_io,
!   nekstab_modal_pod, nekstab_modal_dmd, nekstab_modal_spod, nekstab_modal_spod_streaming,
!   SIZE, TOTAL
!-----------------------------------------------------------------------

module nekstab_modal_analysis
   use nekstab_krylov_subspace, only: krylov_vector, k_zero, k_add2, k_cmult, k_sub2
   use nekstab_vectors, only: nopcopy
   use nekstab_io, only: load_files
   use nekstab_modal_pod, only: pod_compute, pod_fft_spectrum
   use nekstab_modal_dmd, only: dmd_compute
   use nekstab_modal_spod_streaming, only: spod_streaming_batch
   use nekstab_nek_bridge, only: nid, nekStab_error, nekStab_log, &
                                 modal_prefix, modal_nsnap, modal_dt, &
                                 modal_nsave, ifpod, ifdmd, ifspod, ifwinamp, &
...
!        Validate parameters
      if (modal_nsnap < 2) then
         call nekStab_error('ERROR: modal_nsnap must be >= 2')
      end if

      if (.not. ifpod .and. .not. ifdmd .and. .not. ifspod) then
         call nekStab_log('WARNING: No methods enabled (set ifpod/ifdmd/ifspod)')
         ! Note: early return here (no modal methods active); legacy dispatcher style.
         return
      end if
