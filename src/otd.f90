...
!
! Dependencies:
!   nekstab_nek_bridge, krylov_subspace, nekstab_noise
!-----------------------------------------------------------------------

module nekstab_otd
   use nekstab_nek_bridge, only: EIGI, EIGR, EVL, EVR, EVRI, EVRR, EVRi, EVRr, FTLEv, &
                                 FTLEv_prev, LEintegral, LWORKR, OTDfx, OTDfy, OTDfz, &
                                 OTDmix, OTDmiy, OTDmiz, OTDmrx, OTDmry, OTDmrz, RWORK, &
                                 SESSION, VXP, VYP, VZP, bm1, convx, convy, convz, diffx, &
                                 diffy, diffz, dt, dxm1, dxtm1, dytm1, gradpx, gradpy, &
                                 gradpz, gsstep_override, if3d, ifbase, ifield, ifotd, &
                                 ifoutfld, ifpert, initc, istep, jacmi, lastep, ldim, lelv, &
                                 lglel, lpert, lsize, lx1, lx2, ly1, ly2, lz1, lz2, ndim, &
                                 nekStab_error, nekStab_log, nelv, nid, npert, nx1, ny1, nz1, &
                                 otd_FTLEPeriod, otd_Lr, otd_Lux, otd_Luy, otd_Luz, &
                                 otd_convTol, otd_gsStep, otd_idx, otd_minSteps, &
                                 otd_printStep, param, phi_rot, pr, prp, rxm1, rym1, rzm1, &
                                 sxm1, sym1, szm1, t, time, txm1, tym1, tzm1, ubic, uparam, &
                                 upic, v1mask, v2mask, v3mask, vbic, vdiff, vmult, vpic, vx, &
                                 vxp, vxpic, vy, vyp, vypic, vz, vzp, vzpic, wbic, wpic, xm1, &
                                 ym1, zm1
   ...
   !  Note: this indirection is the historical path from the v1 OTD work.
...
!-----------------------------------------------------------------------
   subroutine otd
      ...
      if (.not. OTD_init) then
         ...
         if (file_found) then
            call nekStab_log('OTD: loading base flow from '//trim(filename))
            call load_fld(filename)
            call opcopy(ubic, vbic, wbic, vx, vy, vz)
         else
            call nekStab_log('OTD: no BF_ file found, using useric as base flow')
            call opcopy(ubic, vbic, wbic, vx, vy, vz)
         end if
         ...
         if (latest_restart > 0) then
            call nekStab_log('Found IC files:')
            if (nid == 0) write (6, *) latest_restart
            do mode = 1, npert
               write (mode_tag, '(A1,I2.2)') 'r', mode
               write (filename, '(2A,I5.5)') &
                  trim(mode_tag), trim(SESSION)//'0.f', latest_restart
               call nekStab_log('Looking for ICs in '//trim(filename))
               inquire (file=filename, exist=file_found)
               if (file_found) then
                  call load_fld(filename)
            ...
            call nekStab_log('No IC files found.. using white noise')
         end if
         ...
         elseif (uparam(1) == 5) then
            call nekStab_log('OTD in Frozen baseflow mode!')
            ifbase = .false.
            call bcast(ifbase, lsize)
         end if
...
!-----------------------------------------------------------------------
   subroutine otd_white_noise
      ...
      ! nv from globals (set at init in main)
      ...
      nmin = glmin(vxpic, nv)
      nmax = glmax(vxpic, nv)
      call nekStab_log('OTD noise min,max')
      if (nid == 0) write (6, *) nmin, nmax
...
!-----------------------------------------------------------------------
   real function otd_inner_product(ipert, jpert, iflag)
      ...
      elseif (iflag == 2) then
         ...
         call nekStab_error('Invalid iflag in otd_inner_product')
      end if
...
!-----------------------------------------------------------------------
   subroutine otd_normalize_vector_field(uxp, uyp, uzp)
      ...
      if (norm_sq <= 0.0d0) then
         call nekStab_error('Error in otd_normalize_vector_field (zero norm)')
      end if
...
!-----------------------------------------------------------------------
   subroutine otd_compute_orthonormality_measures( &
      ...
      ! nv from globals; ldv is compile-time max
...
!-----------------------------------------------------------------------
   subroutine otd_compute_eig_wrapper(n, kind)
      ...
      elseif (kind == 'b') then
         ...
         call nekStab_error('ERROR: choose left/right/both eigenvectors: '//kind)
      end if
      ...
      if (info < 0) then
         call nekStab_error('ERROR: the i:th argument had an illegal value in dgeev')
      elseif (info > 0) then
         call nekStab_error('ERROR: the QR algorithm failed in dgeev')
      end if ! info < 0
