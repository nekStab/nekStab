...
!
! Dependencies:
!   nekstab_krylov_subspace, nekstab_fourier
!-----------------------------------------------------------------------
module nekstab_modal_pod

   use nekstab_krylov_subspace, only: krylov_vector, k_zero, k_add2s2, k_cmult, k_norm, &
                                      NEKSTAB_PI
   use nekstab_krylov_inner_products, only: k_gram_matrix
   use nekstab_lapack, only: eig_symmetric
   use nekstab_vectors, only: nopcopy
   use nekstab_nek_bridge, only: nid, vx, vy, vz, pr, t, nekStab_error, &
                                 nekStab_dp, ifwinamp, NEKSTAB_UNIT_POD, &
                                 NEKSTAB_UNIT_SPOD
   use nekstab_fourier, only: fft_r2c
...
!-----------------------------------------------------------------------
   subroutine pod_write_spectrum(eigvals, n, norms, nnorms)
      ...
      open (unit=NEKSTAB_UNIT_POD, file='pod_spectrum.dat', status='replace')
      write (NEKSTAB_UNIT_POD, '(A)') '# POD Eigenvalue Spectrum'
      write (NEKSTAB_UNIT_POD, '(A)') &
      ...
      do m = 1, n
         ...
         if (m <= nnorms) then
            write (NEKSTAB_UNIT_POD, '(I6, 4E16.8)') m, eigvals(m), &
               100.0d0*eigvals(m)/total_energy, &
               100.0d0*cumsum/total_energy, &
               norms(m)
         else
            write (NEKSTAB_UNIT_POD, '(I6, 3E16.8)') m, eigvals(m), &
               100.0d0*eigvals(m)/total_energy, &
               100.0d0*cumsum/total_energy
         end if
      end do

      close (NEKSTAB_UNIT_POD)
...
!  Write temporal coefficients to file
      if (nid == 0) then
         open (unit=NEKSTAB_UNIT_POD, file='pod_coefficients.dat', &
               status='replace')
         write (NEKSTAB_UNIT_POD, '(A)') '# POD Temporal Coefficients'
         write (NEKSTAB_UNIT_POD, '(A,I6,A,I4)') &
            '# nsnap = ', nsnap, ',  nmodes = ', r
         write (NEKSTAB_UNIT_POD, '(A)', advance='no') '# snapshot'
         do k = 1, r
            write (NEKSTAB_UNIT_POD, '(A,I3)', advance='no') '         a_', k
         end do
         write (NEKSTAB_UNIT_POD, *)

         do i = 1, nsnap
            write (NEKSTAB_UNIT_POD, '(I6)', advance='no') i
            do k = 1, r
               write (NEKSTAB_UNIT_POD, '(E14.6)', advance='no') coeffs(i, k)
            end do
            write (NEKSTAB_UNIT_POD, *)
         end do
...
!  Setup FFT parameters
      ...
      if (nblk < 1) then
         call nekStab_error('Not enough snapshots for FFT')
      end if
...
!  Find peaks and write spectrum file
      ...
      if (nid == 0) then
         ...
         open (unit=NEKSTAB_UNIT_SPOD, file='pod_fft_spectrum.dat', &
               status='replace')
         write (NEKSTAB_UNIT_SPOD, '(A)') '# POD-FFT Spectral Analysis'
         write (NEKSTAB_UNIT_SPOD, '(A,I6,A,I6,A,I6,A,I6,A,E12.4)') &
         ...
         do k = 1, npeak
            write (NEKSTAB_UNIT_SPOD, '(A,I3,A,F8.4,A,E12.4)') &
         ...
         do k = 1, r
            write (NEKSTAB_UNIT_SPOD, '(A,I3)', advance='no') '  P_mode', k
         end do
         write (NEKSTAB_UNIT_SPOD, *)

         do i = 1, nfreq
            write (NEKSTAB_UNIT_SPOD, '(E14.6)', advance='no') freq(i)
            do k = 1, r
               write (NEKSTAB_UNIT_SPOD, '(E14.6)', advance='no') power(i, k)
            end do
            write (NEKSTAB_UNIT_SPOD, *)
         end do
