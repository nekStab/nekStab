#!/usr/bin/env python3
"""plot.py: Cylinder Re=180 direct Floquet stability — canonical stability panels.

Panel sequence (stability lane: BF -> SPECTRUM -> MODE):
  plot_bf.png       - UPO base flow (BF_*0.f00001 if present, else skipped)
  plot_spectrum.png - Floquet spectrum on unit circle (Spectre_Hd.dat)
  plot_mode.png     - direct Floquet mode (dRe1cyl0.f00001, vy field, RdBu)
...
def find_mode(case_dir):
    """Find direct Floquet mode real part."""
    matches = sorted(case_dir.glob('dRe*0.f00001'))
    if not matches:
        matches = sorted(case_dir.glob('dRe*0.f0*'))
...
def main():
    ...
    # PANEL 2 — SPECTRUM (Floquet unit circle)
    spec_path = CASE_DIR / 'Spectre_Hd.dat'
    if not spec_path.exists():
        # fallback to NS direct spectrum
        spec_path = CASE_DIR / 'Spectre_NSd.dat'
    if spec_path.exists():
