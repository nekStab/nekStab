#!/usr/bin/env python3
"""plot.py: Visualize lid-driven cavity Newton base flow results.
...
def main():
    ...
    bf_files = nk.find_fields('BF_cav0.f*', CASE_DIR)

    if has_conv and bf_files:
        fig = plt.figure(figsize=(nk.COL_WIDTH * 2, nk.COL_WIDTH * 0.55))
        gs = fig.add_gridspec(1, 2, width_ratios=[1, 1], wspace=0.35)
        ax_conv = fig.add_subplot(gs[0])
        ax_bf = fig.add_subplot(gs[1])
    elif bf_files:
        fig, ax_bf = plt.subplots(1, 1, figsize=(nk.COL_WIDTH * 0.6, nk.COL_WIDTH * 0.6))
        ax_conv = None
    else:
