#!/usr/bin/env python3
"""plot.py: Thermosyphon Ra=500 Newton base flow — canonical baseflow panels.
...
Panel sequence (canonical baseflow lane: IC -> RESIDUAL -> BF):
  ...
  plot_bf.png       - converged base flow temperature (renamed from plot_baseflow)

Geometry: annulus, R_inner=1, R_outer=2. Field: temperature 't', cmap='inferno'.
...
def find_bf(case_dir):
    for pat in ('BF_tsyphon0.f00001', 'BF_Ra*tsyphon0.f00001',
                'BF_*tsyphon0.f*', 'BF_*0.f00001'):
...
def render_annulus_field(ax, field_path):
    x, y, fields, _ = nk.read_field(str(field_path))
    triang = nk.make_triangulation(x, y)
    q = fields.get('t')
    if q is None:
        # Fallback to velocity magnitude if no temperature
    ...
    nk.inset_colorbar(ax, cf, orientation='horizontal',
                      ...
                      ticks=[0, 1], tick_labels=['0', '1'])
    ax.axis('equal')
    nk.add_annulus_patches(ax, R_INNER, R_OUTER)
    _setup_annulus_axes(ax)
...
def main():
    ...
    ic = find_ic(CASE_DIR)
    if ic:
        fig, ax = plt.subplots(1, 1, figsize=(nk.COL_WIDTH * 0.5, nk.COL_WIDTH * 0.5))
        render_annulus_field(ax, ic)
    ...
    # PANEL 3 — BF (converged base flow temperature)
    bf = find_bf(CASE_DIR)
    if bf:
        fig, ax = plt.subplots(1, 1, figsize=(nk.COL_WIDTH * 0.5, nk.COL_WIDTH * 0.5))
        render_annulus_field(ax, bf)
