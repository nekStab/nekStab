#!/usr/bin/env python3
"""plot.py: Visualize adjoint stability analysis results.
...
def main():
    ...
    if ax_spec is not None and has_spec:
        ...
        ax_spec.set_title(r'$\sigma$ vs $f$ (adjoint)', fontsize=8)
        nk.panel_label(ax_spec, rf'$\bf{{({next(labels)})}}$')

    if ax_mode is not None and has_mode:
        x, y, fields, time = nk.read_field(aRe_files[0])
