...
def main():
    ...
    if has_mode:
        _x, _y, fields_preview, _time = nk.read_field(dRe_files[0])
        has_temp = 't' in fields_preview

    if has_spec and has_mode and has_temp:
        fig = plt.figure(figsize=(nk.COL_WIDTH * 3, nk.COL_WIDTH * 0.45))
        gs = fig.add_gridspec(1, 3, width_ratios=[0.8, 1, 1], wspace=0.35)
        ax_spec = fig.add_subplot(gs[0])
        ax_mode = fig.add_subplot(gs[1])
        ax_temp = fig.add_subplot(gs[2])
    elif has_spec and has_mode:
        fig = plt.figure(figsize=(nk.COL_WIDTH * 2, nk.COL_WIDTH * 0.45))
        gs = fig.add_gridspec(1, 2, width_ratios=[0.8, 1], wspace=0.35)
        ax_spec = fig.add_subplot(gs[0])
        ax_mode = fig.add_subplot(gs[1])
        ax_temp = None
    elif has_mode and has_temp:
        fig = plt.figure(figsize=(nk.COL_WIDTH * 2, nk.COL_WIDTH * 0.45))
    ...
    if ax_spec is not None and has_spec:
        nk.plot_spectrum_NS_paper(ax_spec, spec_ns, label=r'$Re=30$')
    ...
    if ax_mode is not None and has_mode:
        ...
        if ax_temp is not None and 't' in fields:
            q_t = fields['t']
            bd_t = np.nanpercentile(np.abs(q_t), 99)
            cf_t = nk.tricontourf(ax_temp, triang, q_t, levels=257,
                                  cmap='RdBu', vmin=-bd_t, vmax=bd_t, extend='both')
            bdt = round(bd_t, 2)
            nk.inset_colorbar(ax_temp, cf_t, orientation='horizontal',
                              width="50%", height="5%", loc=9,
                              ticks=[-bdt, bdt],
                              tick_labels=[f'{-bdt}', f'{bdt}'])
            ax_temp.set_aspect('equal')
            nk.add_cylinder_patches(ax_temp)
            ax_temp.set_xlim(-2, 20)
            ax_temp.set_ylim(-4, 4)
            ax_temp.set_xlabel(r'$x$', labelpad=-1)
            ax_temp.spines['right'].set_visible(False)
            ax_temp.spines['top'].set_visible(False)
            nk.panel_label(ax_temp, rf'$\bf{{({next(labels)})}}$')
