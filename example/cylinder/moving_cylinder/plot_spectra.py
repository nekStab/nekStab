...
# Read A value from 1cyl.par file
def read_A_param():
    with open('1cyl.par', 'r') as f:
        for line in f:
            if 'userParam05' in line:
                match = re.search(r'userParam05\s*=\s*([0-9.]+)', line)
                if match:
...
if __name__ == '__main__':
    # Get the A value from 1cyl.par
    A_value = read_A_param()
    A_label = rf'$A={A_value}$' if A_value is not None else r'$A=?$'
    A_str = f"A{A_value}" if A_value is not None else "A_unknown"
    ...
    plot_H(plt, f.R, f.I, f.r, 8, 'k', 'o', A_label)

    fname=f'Spectre_H_{A_str}.{formt}'
    ...
    plot_NS(plt, f.R, f.I, f.r, True, 8, 'k', 'o', A_label)

    fname=f'Spectre_NS_{A_str}.{formt}'
    plt.legend(loc='best',fontsize=6)
    plt.savefig(fname,format=formt,dpi=qual,bbox_inches=ajust);print('Saving '+fname);plt.close()
    print('------------------------------------------')
