...
def main():
    ...
    tskps = [200.0] # set to 0 to plot all
    ...
    for i, filen in enumerate(files):
        ...
        for prb in range(0,1,1):
            ...
            ref = 7.205882352941176e-01/(2.0*np.pi)
            axs[1].axvline(x=ref,c='k',lw=0.5,ls='--',label='$St=$'+("{0:.4f}".format(round(ref,4))))
            percentage_difference = ((prfreq - ref) / ref) * 100
            print(prfreq,ref,percentage_difference)
