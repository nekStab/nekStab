...
def process_file(filename, tskip=0, tmax=0):
    ...
    try:
        data = data.reshape(-1, nps, 4)
        is_3d = False
    except:
        data = data.reshape(-1, nps, 5)
        is_3d = True
...
def crop_and_interpolate(t, u, v, w, tmin, tmax):
    tmin = 0 if tmin > t.min() else tmin
...
def plot_time_series_and_fft(t, v, coords, prb):
    ...
    plt.tight_layout()
    filename = f'his{prb+1:02}_fft.{format}'
...
def plot_phase_spaces(u, v, w, prb, is_3d):
    if is_3d:
        ...
        plt.tight_layout()
        filename = f'his{prb+1:02}_phase_space_3d.{format}'
    else:
        ...
        plt.tight_layout()
        filename = f'his{prb+1:02}_phase_space_2d.{format}'
...
if __name__ == "__main__":
    ...
    tskip = 400  # You can modify this value if needed
