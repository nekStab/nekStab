...
for filen in files:
    ...
    for prb in range(0,nps,1):
        ...
        axs[1].axvline(x=prfreq         ,c=cor,lw=0.5,ls='--',label='$St_0=$'+("{0:.4f}".format(round(prfreq,4))))
        prfreq2 = ufreq[np.argmax(np.abs(ufft[ufreq<0.06]))]
        axs[1].axvline(x=prfreq2         ,c=cor,lw=0.5,ls='-.',label='$St_1=$'+("{0:.4f}".format(round(prfreq2,4))))
        axs[1].axvline(x=99999         ,c=cor,lw=0.5,ls='-.',label='$St_0/St_1=$'+("{0:.4f}".format(round(prfreq/prfreq2,4))))
