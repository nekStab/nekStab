      ...
      subroutine otd_init
         ...
         call white_noise_IC ! Initialise all IC fields to white noise
            if (otd_nusric < 0) then !  Read IC fields
               if (nid == 0) write (6, *) 'Choose a valid number of IC files.'
               call nek_end
            else if(otd_nusric == 0) then
               if (nid == 0) write (6, *) 'No IC files read. ICs will be white noise.'
            else

            if(otd_nusric > npert .and. nid == 0) write (6, *) 'otd_nusric > npert! Increase the number of perturbations.'

            ! save mesh
            call opcopy(xtmp,ytmp,ztmp,xm1,ym1,zm1)
            ! read ICs

            if (nid == 0) write (6, *) 'Reading ICs'
            call read_OTDIC()
            ! restore mesh
         ...
         !   Sanity check
           if (otd_ifFTLE) then
             ...
             if (nid == 0) write (6, *)'The FLTEs will be computed continuously. Set otd_FTLEpd in the par-file to define a finite time horizon.'
             endif
             pcount = 0
           endif
         ...
         !   Ensure right variable size
           if (npert.gt.lpert) then
            if (nid == 0) write (6, *) 'npert > lpert! Set lpert=npert for the tool to work.'
           endif
...
!       Compute additional forcing for perturbation equation
            ...
            if (mod(istep, otd_prntsp) == 0) call proj_OTDmodes ! Compute the OTD modes 
            if (mod(istep, otd_iostep) == 0) call outpost_projmodes ! Output OTD modes
...
!
      subroutine read_OTDIC()
         ...
         if (nid == 0) write (6, *)'INIT - read ICs'
         do i = 1, otd_nusric
            write (istr, '(I0.2)') i
            ifile = 'OTDIC_'//trim(istr)//'.fld'
            inquire (file=ifile, exist=exist_IC)
            if (exist_IC) then
               if (nid == 0) write (6, *) 'INIT - Reading IC file '//trim(ifile)
               call load_fld(ifile)
               ...
               if (nid == 0) write (6, *) 'Cannot open '//trim(ifile)//' !'
            end if
         end do
         if (nid == 0) write (6, *) 'INIT - read ICs :: done'
...
!==================================================
      ...
      subroutine gen_OTD_forcing() ! Create forcing for OTD evolution equation
         implicit none
         include 'SIZE'
         include 'SOLN'
         include 'OTD'
         ...
         integer nv
         nv = lx1*ly1*lz1*lelv
         call mxm(VXP, nv, Lr, lpert, OTDfx, npert)
         call mxm(VYP, nv, Lr, lpert, OTDfy, npert)
         call mxm(VZP, nv, Lr, lpert, OTDfz, npert)
...
!=======================================================================
      ...
      subroutine reset_FTLE() ! Reset FTLE computation
         ...
         !call ident(Phi,lpert) ! Phi
         call rzero(trapz, lpert) ! Blanchard
...
!=======================================================================
      ...
      ! 1. Advect fundamental solution matrix Phi : dPhi/dt = Lr    , with Phi(t=t0) = I(rxr)
      ! 2. Compute FTLEs : FTLE(i) = 1/T * log(svd(Phi(t)))
...
!=======================================================================
      ...
      subroutine compute_FTLE_Blanchard(Lrp, Lrc, deltat, ftledt) ! Compute FTLEs (Based on Blanchard & Sapsis, 2019)
         implicit none
         include 'SIZE'
         include 'TSTEP'
         include 'OTD'
...
!=======================================================================
      subroutine get_FTLE()
         ...
         call copy(Lrp, Lr, lpert*lpert) ! update Lrp
...
!======================================================================
      ...
      real function op_ip_vp(vcx, vcy, vcz, jpert)
         ...
         real op_glsc2_wt
         ifield = 1
         op_ip_vp = 0.50d0*op_glsc2_wt(VCX, VCY, VCZ, VXP(1, jpert), VYP(1, jpert), VZP(1, jpert), bm1)
         return
...
!======================================================================
      ...
      real function op_ip(ipert, jpert, iflag)
         ...
         elseif (iflag == 2) then
            ...
            if (nid == 0) write (6, *) 'Error in op_ip!'
            call nek_end
         end if
...
!======================================================================
     ...
     $   , uzp(lx1*ly1*lz1*lelv, 1)
         ...
         if (n2 <= 0.0) then
            if (nid == 0) write (6, *) 'Error in op_norm!'
            call nek_end
         end if
         invnorm = 1.0d0/sqrt(n2)
         call cmult(uxp, invnorm, nv)
         call cmult(uyp, invnorm, nv)
         if (if3d) call cmult(uzp, invnorm, nv)
...
!
         ...
         do i = 1, npert
            do j = 1, i - 1
               ...
               call add2s2(vxp(1, i), vxp(1, j), proj, nv)
               call add2s2(vyp(1, i), vyp(1, j), proj, nv)
               if (if3d) then
                  call add2s2(vzp(1, i), vzp(1, j), proj, nv)
               end if
            end do
            invnorm = 1.0d0/sqrt(op_ip(i, i, 1))
            call cmult(vxp(1, i), invnorm, nv)
            call cmult(vyp(1, i), invnorm, nv)
            if (if3d) then
               call cmult(vzp(1, i), invnorm, nv)
            end if
         end do

         if (nid == 0) write (6, *) 'V[XYZ]P orthonormalized (CGS)',istep
...
!
   ...
   !-----------------------------------------------------------------------
         ...
         if (nid == 0) write (6, *) 'V[XYZ]P orthonormalized (MGS)',istep
         return
      end subroutine MGS
!
!======================================================================

      subroutine compute_NO(N, O, info, flag) !Compute measures of orthogonality and normality of the perturbations
         implicit none
         include 'SIZE'
         include 'TSTEP'
         include 'OTD'
