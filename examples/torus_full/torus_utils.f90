     ...
     $                          VX_TPER,VY_TPER,VZ_TPER,T_TPER,BM1)
      ...
      j0_b   = jn_b(1)
      j1_b   = jn_b(2)
      binv_pulse   = 1./(2.*j1_b/(j0_b*wwi)-1.)
      
      fshape = 1./sqrt((1+delta*sqrt(rr)*sin(alpha))**2)
      dpds_steady = 0.159786157399786
      !      dpdt = real((0.012993982565043 + 0.154452897033316*imag)
      !     $          *cexp(imag*omega*time))
      dp1ds = 2.*real( (dp1dsr + dp1dsi*imag)*cexp(imag*omega*time) )
      !      dpds = (dpds_steady + 2.*dpdt)*fshape/curv_radius
